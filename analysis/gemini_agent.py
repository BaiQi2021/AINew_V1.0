...
"""
    ...
    def _fix_markdown_format(self, content: str) -> str:
        ...
        # 1. 首先进行缩进规范化
        content = self._normalize_markdown_indentation(content)
        ...
        def replace_func(match):
            ...
            # 去掉 bullet_part 末尾可能存在的冒号
            clean_bullet = re.sub(r'[:：]\s*$', '', bullet_part)
            
            # 另起一行并缩进 4 空格
            return f"{indent}{clean_bullet}\n{indent}    {text}"
    ...
    def _normalize_markdown_indentation(self, content: str) -> str:
        """
        规范化 Markdown 列表的缩进，确保平级项缩进一致。
        """
        lines = content.split('\n')
        new_lines = []
        ...
        for line in lines:
            ...
            if stripped.startswith(('-', '*')):
                ...
                # 规范化：每 4 个空格一级
                level = (indent_len + 1) // 4
                new_indent = "    " * level
                new_lines.append(new_indent + stripped)
            elif line.strip() == "":
                new_lines.append("")
            else:
                # 对于非列表行，如果是内容的延续，也保持规范化缩进
    ...
    def _validate_news_item_format(self, content: str) -> Tuple[bool, str]:
        ...
        required_patterns = [
            (r"### \*\*.*?\*\*", "标题格式错误，应为 ### **标题**"),
            # (r"\[阅读原文\]\(.*?\)", "缺少阅读原文链接 or 格式错误"), # 阅读原文必须存在
            (r"> \*\*概要\*\*[:：].*", "缺少概要或格式错误"),
            (r"\*\*💡内容详解\*\*", "缺少'💡内容详解'分节"),
            (r"- \*\*.*?\*\*", "缺少要点标题或格式错误")
        ]
        ...
        if re.search(r"### \*\*\[.+?\]\*\*", content):
            return False, "标题格式错误：不应使用方括号[]，应为 ### **标题** 而非 ### **[标题]**"
        
        # 检查是否有阅读原文链接 (必须存在)
        if not re.search(r'\[阅读原文\]\(', content):
             return False, "缺少[阅读原文]链接，请添加链接"
...
**检查规则：**
1. **标题**：必须以 `### **` 开头，`**` 结尾。
2. **阅读原文**：必须包含 `[阅读原文](URL)  `[YYYY-MM-DD]` `。
3. **概要**：必须以 `> **概要**：` 开头。
4. **内容详解**：必须包含 `**💡内容详解**` 标题。下方应为列表结构（`- **...**`）。
5. **分行要求**：标题与解释内容**必须分行**，解释内容必须有 4 空格缩进，**严禁**在标题行直接写内容或使用冒号。
6. **层级一致性**：检查列表缩进是否统一（大标题不缩进，子项缩进 4 空格）。
7. **关键点**：必须包含至少一个关键点大标题（`- **...**`）。
6. **相关论文**：
...
2. **生成Markdown内容**：严格遵循以下Markdown格式模板生成内容。
   ...
   **💡内容详解**
   ...
   - **关键点大标题 X**
       在此处详细解释该技术。如果该关键点包含多个子项，请使用下方格式：
       - **子关键点 A**
           子项详细解释，要有具体技术细节，不超过200字。
   ```

   **格式要求（重要）：**
   1. **层级一致性**：平级的关键点必须使用相同的缩进量（推荐 0 缩进用于大标题，4 空格缩进用于子项）。
   2. **内容详解**：下方必须紧跟列表项，不能有任何正文段落。
   3. **标题与内容分行**：**严禁**使用冒号连接标题与内容。**必须**将解释内容另起一行，并增加 4 个空格的缩进。

   **关于 [阅读原文] 的特别说明：**
   - 必须生成 `[阅读原文]([primary_url])  `[YYYY-MM-DD]` ` 这一行。
   ...
   - **即使 primary_url 是论文链接，也必须生成 [阅读原文] 这一行。**
...
2. **生成Markdown内容**：严格遵循以下Markdown格式模板生成内容。
   ...
   **关于 [阅读原文] 的特别说明：**
   - 必须生成 `[阅读原文]([URL])  `[YYYY-MM-DD]` ` 这一行。
   - 必须使用提供的 url（已按优先级排序：官方信源 > 技术媒体 > 普通媒体）。
   - **信源优先级说明：** 官方信源（openai.com, anthropic.com, deepmind.google 等）优先级最高，其次是技术平台（github, arxiv），最后是媒体报道。
   - **允许使用非官方信源：** 如果没有官方信源，可以使用量子位、36kr等媒体链接作为阅读原文。
   - **即使 url 是论文链接，也必须生成 [阅读原文] 这一行。**
