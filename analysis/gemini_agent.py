...
"""
    ...
    def _validate_news_item_format(self, content: str) -> Tuple[bool, str]:
        ...
        required_patterns = [
            (r"### \*\*.*?\*\*", "标题格式错误，应为 ### **标题**"),
            # (r"\[阅读原文\]\(.*?\)", "缺少阅读原文链接 or 格式错误"), # 阅读原文现在是可选的（量子位链接不显示）
            (r"> \*\*概要\*\*[:：].*", "缺少概要或格式错误"),
            (r"\*\*💡内容详解\*\*", "缺少'💡内容详解'分节"),
            (r"- \*\*.*?\*\*", "缺少要点标题或格式错误")
        ]
        ...
        # 检查是否只有日期而没有阅读原文链接
        # 匹配独立的日期行 (如 `[2025-12-30]`) 但没有 [阅读原文]
        date_only_pattern = r'^\s*`\[\d{4}-\d{2}-\d{2}\]`\s*$'
        if re.search(date_only_pattern, content, re.MULTILINE):
            # 检查是否有阅读原文链接
